...
func (c *Client) EnhanceSearchQuery(ctx context.Context, query string) ([]string, error) {
	...
	prompt := fmt.Sprintf(`Проанализируй поисковый запрос и помоги улучшить его для поиска социальных льгот, коммерческих скидок и мер поддержки.

Запрос: "%s"

Твоя задача:
1. ОБЯЗАТЕЛЬНО исправь орфографические ошибки и опечатки (например, "оптека" -> "аптека")
2. Добавь морфологические варианты слов (например, "аптека" -> "аптека, аптеки, аптек, аптеке, аптекой")
3. Найди ТОЛЬКО релевантные синонимы и связанные термины, которые относятся к теме запроса
4. НЕ добавляй общие термины, которые не связаны с темой запроса
5. Для организаций добавь их названия (например, "аптека" -> "Твоя Аптека, аптечная сеть")

ВАЖНО: 
- Если в запросе есть опечатки, ВСЕГДА начинай список с правильного написания слова
- Фокусируйся ТОЛЬКО на теме запроса. Например, для "здоровье" используй: здоровье, медицина, лечение, врач, больница, лекарства, аптека, медицинские услуги
- Для запросов о здоровье, медицине, лечении ОБЯЗАТЕЛЬНО включай "аптека" и связанные термины (аптеки, аптечная сеть, лекарства), так как аптеки являются важной частью системы здравоохранения
- НЕ добавляй общие термины типа "льготы", "скидки", "пенсионеры", "ветераны", если они не относятся к теме запроса
...
Запрос "оптека" -> аптека, аптеки, аптек, фармация, лекарства, медикаменты, Твоя Аптека
Запрос "студент" -> студент, студенты, студентам, учащийся, обучающийся
Запрос "здоровье" -> здоровье, медицина, лечение, врач, больница, лекарства, аптека, аптеки, медицинские услуги, здравоохранение, аптечная сеть
